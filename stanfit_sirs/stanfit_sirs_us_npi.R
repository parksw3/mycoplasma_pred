...
Npred <- 52*10
Nnonpi <- nrow(filter(data_myco_gather, year + week/52  < 2020 + 12/52))
...
standata <- list(
  ...
  positivity=data_myco_gather$value,
  mu=1/80/52,
  pop=1,
  gamma=1/3
)

stanfit_sirs_us_npi <- sampling(model,
															 data=standata,
															 seed=101,
															 chain=4,
															 cores=4,
...
check_hmc_diagnostics(stanfit_sirs_us_npi)
get_num_divergent(stanfit_sirs_us_npi)
get_num_max_treedepth(stanfit_sirs_us_npi)
get_low_bfmi_chains(stanfit_sirs_us_npi)
get_bfmi(stanfit_sirs_us_npi)

save("stanfit_sirs_us_npi", file="stanfit_sirs_us_npi.rda")

ss <- summary(stanfit_sirs_us_npi)

max(ss$summary[which(!is.na(ss$summary[,10])),10]) ## 1.004842
min(ss$summary[which(!is.na(ss$summary[,10])),9]) ## 612.5568

plot(1:(N+Npred)/52, ss$summary[grepl("Ifit\\[", rownames(ss$summary)),6], col=2)

plot(c(rep(NA, 12), zoo::rollmean(data_myco_gather$value, 12)), type="l", xlim=c(0, 1000))
